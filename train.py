...
def setup(args):
    ...
    model = VisionTransformer(config, args.img_size, zero_head=True, num_classes=num_classes, vis=True,
                              smoothing_value=args.smoothing_value, dataset=args.dataset)
...
def main():
    ...
    parser.add_argument("--dataset",
                        choices=["cifar10", "cifar100", "ip_102", "AgriculturalDisease", "PlantLeave", "PlantPathology", "PlantDoc","AppleLeaf9", "AppleLeaf", "soyloc", "cotton", "CUB",
                                 "dog", "car", "air"],
                        default="cotton",
    ...
    #
    # model.load_state_dict(torch.load('./output/AgriculturalDisease_LOG_checkpoint.bin'),False)
    #
    # torch.save(model, './output/AgriculturalDisease_checkpoint.pkl')
    #
    # image = Image.open('F:/DataSet/fine-grained/AgriculturalDisease/valid/images/43234193db4aefa1245592ab36d6c946.jpg')
    #
    # image_tensor = test_transform(image)
    #
    # image_tensor = image_tensor.reshape(1, 3, image_tensor.size()[1], image_tensor.size()[2]).to('cuda')
    #
