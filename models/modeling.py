...
class Attention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        # 假设多头注意力头数为12， 则(32, 197, 768) -> (32, 197, 12, 64)
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
        x = x.view(*new_x_shape)
        # (32, 197, 12, 64) -> (32, 12, 197, 64)
        return x.permute(0, 2, 1, 3)

    def forward(self, hidden_states):
        # hidden_states: (32, 197, 768)
        # self.query\key\value是各自的映射，输出为(32, 197, 768)
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)

        # (32, 197, 768) -> (32, 12, 197, 64)
        # (32, 12, 197, 64)中12表示12个注意力头，197是嵌入向量的个数。计算时，计算每个注意力头中的不同嵌入向量间的关系
        query_layer = self.transpose_for_scores(mixed_query_layer)
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)

        # (32, 12, 197, 64) -> (32, 12, 197, 197)
        ...
        # (32, 12, 197, 197) -> (32, 12, 197, 64)
        context_layer = torch.matmul(attention_probs, value_layer)
        # (32, 12, 197, 64) -> (32, 197, 12, 64)
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        # 合并所有注意力头：(32, 197, 12, 64) -> (32, 197, 768)
        context_layer = context_layer.view(*new_context_layer_shape)
        # (32, 197, 768)
        attention_output = self.out(context_layer)
        attention_output = self.proj_dropout(attention_output)

        # print('111111',weights.size())
        # print('222222',self.softmax2(attention_scores)[:,:,:,0].size())

        # (32, 197, 768), (32, 12, 197, 197), (32, 12, 197)
        # self.softmax2(attention_scores)[:, :, :, 0]：相当于论文中提出的权重矩阵的第一列 b0
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.patches.get("grid") is not None:
            ...
            # 假设这里patch_size大小：16*16
            patch_size = _pair(config.patches["size"])
            # patch的数量：(224/16)*(224/16) = 14*14
    ...
    def forward(self, x):
        # x:(B, C, H, W) 这里以(32, 3, 224, 224)为例
        B = x.shape[0]
        # (1, 1, hidden_size) -> (B, 1, hidden_size) 这里以(32, 1, 768)为例
        cls_tokens = self.cls_token.expand(B, -1, -1)

        if self.hybrid:
            x = self.hybrid_model(x)

        # patch_embeddings：(32, 3, 224, 224) -> (32, 768, 14, 14)
        x = self.patch_embeddings(x)
        # flatten(2): (32, 768, 14, 14) -> (32, 768, 196)
        x = x.flatten(2)
        # transpose(-1, -2): (32, 768, 196) -> (32, 196, 768)
        x = x.transpose(-1, -2)
        # (32, 196, 768) -> (32, 197, 768)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # x:(32, 197, 768), weights:(32, 12, 197, 197), contribution:(32, 12, 197)
        ...
        x = x + h
        # x:(32, 197, 768), weights:(32, 12, 197, 197), contribution:(32, 12, 197)
        # contribution代表的是论文中提到的注意力分数第一列 b0
...
class MAWS(nn.Module):
    ...
    def forward(self, x, contributions):
        # x:(32, 12, 197, 197)
        # contributions:(32, 12, 197)

        # 对所有头的注意里分数进行平均 (32, 12, 197) -> (32, 197), 即得到 b0
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            # hidden_states:(32, 197, 768), weights:(32, 12, 197, 197), contribution:(32, 12, 197)
            ...
            if self.feature_fusion:
                ...
                for i in range(B):
                    # selected_inx[i, :self.num_token]:选择出前num_token个最大值的索引 (1, num_token)
                    # hidden_states[i, selected_inx[i, :self.num_token]: (num_token, 768)
                    # (32, num_token * layer层数, 768)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            # (32, num_classes), (12, 32, 12, 197, 197)
...
def con_loss_1(features, labels):
    B, _ = features.shape
...
def con_loss(features, labels):
    ...
    mask = torch.eye(features.size(0)).cuda().type(torch.bool)
